...
void write_ply(float** R, pt* pts, char* outfile_name, int n, int m) {
    ...
    for (i=0; i < n*m; i++) {
        ...
        if (pts[i].valid) {
            ...
            // write to p_out given p_in and the
            // rotation matrix
            for (j=0; j < 3; j++) {
                for (k=0; k < 3; k++) {
...
float** get_rough_transform(candidate* scores, pt* A, pt* B, int n, int m, int n2, int m2,
    ...
    // perform matrix multiplication to get the output matrix R - the rotation
    for (i=0; i < 3; i++) {
        for (j=0; j < 3; j++) {
            for (k=0; k < 3; k++) {
                R[i][j] += v[i][k] * H[j][k];
...
float hist_dist(float* hA, float* hB, int i, int j) {
    ...
    // if hA at i OR hB at j is negative,
    // set score to 2 - worst score
    if ((hA[i] < 0) || (hB[j] < 0)) {
        return 2;
    }


    // find the score of hA and hB by computing
    // the distance between the two. The sum of 
    // squared difference
    sum = 0;
    for (k=0; k<16; k++) {
...
candidate* score_hists(float* cur_hists, float* base_hists, int n, int m, int n2, int m2) {
    ...
    candidate* scores;
    // malloc the appropriate amount of space for the array of scores
    scores = (candidate*)malloc((CANDIDATES*sizeof(candidate)));

    // initialize all candidates to have a score of 2
    for(i=0; i < CANDIDATES; i++){
        scores[i].score = 2;
    }
    ...
    // for each possible combination of histograms
    // consider the candidate, and compute the score
...
float* generate_histograms(pt* points_list, pt* normals, int n, int m) {
    ...
    // malloc space for an array of histograms
    // so that each vertex has a histogram
    ...
    for (i=0; i< n; i++) {
        for (j=0; j<m; j++) {
            ...
            // initialize point pair tally to 0
            point_pair_tally = 0;
            ...
            */
            for (k = 0; k < K_POINTS - 1; k++) {
                for (l = k + 1; l < K_POINTS; l++) {
                   ...
                   // find the given point on points_list that k, l are at
                    ...
                    if (pt_k.valid &&
                        ...
                        // compute each feature f0, f1, f2, f3
                        // based on specifications found within Rusu paper
                        f[0] = Dot(v, normal_t);
                        ...
                        // sum all the features together into idx
                        ...
                        // increment specific histogram 
                        histograms[((i*m + j) * 16) + idx]++;

                        // increment point_pair_tally
                        point_pair_tally++;
                    }
                }
            } // Back out, @ (i,j)
...
pt* run_pca(pt* points, int n, int m, int v) {
    ...
    for (i=0; i < n; i++) {
        for (j=0; j<m; j++) {
            ...
            // if we haev enough points to approximate a plane, run PCS on kernel points
            // to get the normal vector V
            if (ind >= 3) {
...
pt* parse_ply_pts(char* infile_name, float*** vertices, int* n, int* m, int* v) {
    ...
    *vertices = (float**) malloc(*v * sizeof(float *));
    ...
    for (i = 0; i < *v; i++) {
        ...
        // write the point into the v_array
...
// hashFeatures - given an array of float feature values
// compute idx 
int hashFeatures(float* f, float* s) {
