...
/* for finding out how big things are */
  ...
  /* initialize Tcl */
  setup_tcl(argc, argv);
  add_update (&update_continuously, check_tk, NULL);
  ...
  while(--argc > 0 && (*++argv)[0]=='-') {
    for(s = argv[0]+1; *s; s++)
	  ...
	  strcpy (filename, *++argv);
        ...
        case 'f':
	  strcpy (filename, *++argv);
	  Tcl_VarEval(interp, "source ", filename, (char *) NULL);
          argc--;
          break;
        case 'm':
...
/******************************************************************************
Check to see if there are any Tk events to process, and if there are, do them.
******************************************************************************/
...
void check_tk()
{
  ...
  /* process all waiting Tk events */
  do {
    result = Tk_DoOneEvent (TK_DONT_WAIT);
  } while (result);
}
