...
typedef struct Edge;  /* forward declaration */
...
/******************************************************************************
Set the value of the color to draw.
******************************************************************************/

set_drawing_color(flag,c1,c2,c3)
  int flag;
  int c1;
  int c2;
  int c3;
...
/******************************************************************************
Draw a smaller version of a polygon.

Entry:
  poly  - the polygon to draw
  color - color to draw polygon in
  scale - how much to shrink polygon
******************************************************************************/

draw_small_poly(poly,color,scale)
  Npoly *poly;
  int color;
  float scale;
{
...
fill_circle(x,y,color)
  float x,y;
  int color;
{
#if 0
  draw_circle ((int) x, (int) y, 7, color);
  draw_circle ((int) x, (int) y, 6, color);
  draw_circle ((int) x, (int) y, 5, color);
  draw_circle ((int) x, (int) y, 4, color);
  draw_circle ((int) x, (int) y, 3, color);
  draw_circle ((int) x, (int) y, 2, color);
  draw_circle ((int) x, (int) y, 1, color);
#endif
}


/******************************************************************************
Draw lots of stuff, for debugging.
******************************************************************************/
...
/******************************************************************************
Draw a big fat pixel.
******************************************************************************/

draw_fat_pixel(x,y,col)
  int x,y;
  int col;
{
  color (col);
  recti (x-1, y-1, x+1, y+1);
}
  ...
  int color;
{
  ...
  line ((int) points[n1]->pos[X], (int) points[n1]->pos[Y],
	(int) points[n2]->pos[X], (int) points[n2]->pos[Y], color);
}


/******************************************************************************
Draw a smaller version of a triangle.

Entry:
  p1,p2,p3 - indices of the triangle vertices
******************************************************************************/

draw_small_tri(p1,p2,p3,color)
  int p1,p2,p3;
  int color;
{
  float *v1,*v2,*v3;
...
/******************************************************************************
Show information about the boundary of a polygon.
******************************************************************************/
...
show_boundary()
{
  ...
  for (i = 0; i < npoints; i++) {
    printf ("pnt: %f %f\n", points[i]->pos[X], points[i]->pos[Y]);
    line ((int) points[i]->pos[X], (int) points[i]->pos[Y],
          (int) points[(i+1)%npoints]->pos[X],
	  (int) points[(i+1)%npoints]->pos[Y], color1);
  }
